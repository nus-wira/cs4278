...
class Planner(object):
    ...
    def map_callback(self):
        ...
        self.aug_map = list(copy.deepcopy(self.map))

        # inflate accounting for robot size and map resolution
        total_inflate = int(ROBOT_SIZE / self.resolution)

        # inflate by the robot_size in all 4 directions from each occupied point
        d = ((0,1),(0,-1),(1,0),(-1,0))
        
        for j in range(self.world_height):
            for i in range(self.world_width):
                val = self.map[j * self.world_width + i]
                if val <= 0: 
                    continue
                
                for k in range(1, self.inflation_ratio + 1):
                    for di, dj in d:
                        ni = i + k * di * total_inflate
                        nj = j + k * dj * total_inflate
                        if not self.is_valid(ni, nj):
                            continue
                        self.aug_map[nj * self.world_width + ni] = val
        self.aug_map = tuple(self.aug_map)
    
    def is_valid(self, i, j):
        return 0 <= i < self.world_width and 0 <= j < self.world_height
    ...
    def collision_checker(self, x, y):
        ...
        r_i = ROBOT_SIZE / self.resolution
        r_i = int(r_i / 2) + 1
        for di in range(-r_i, r_i + 1):
            for dj in range(-r_i, r_i + 1):
                try:
                    if self.get_by_index(i + di, j + dj) > 0:
                        return True
                except IndexError:
                    return True
        return False
    ...
    def get_by_index(self, i, j):
        if not self.is_valid(i, j):
            raise IndexError()
        return self.aug_map[j * self.world_width + i]
...
def create_planner(width, height, resolution, inflation_ratio, goal, publish=True):
...
def save_action(task, mapname, goal, planner):
    if task == 1 or task == 2:
        ...
        np.savetxt(
            "{task}_{mapname}_{g0}_{g1}.txt".format(
                task=task, 
                mapname=mapname,
                g0=goal[0],
                g1=goal[1]
        ...
        dump_action_table(
            ...
            "{task}_{mapname}_{g0}_{g1}.json".format(
                task=task, 
                mapname=mapname,
                g0=goal[0],
                g1=goal[1]
...
def main(create_planner_fn, task=1):
    ...
    parser.add_argument('-i', '--inflation', type=int, default=1,
    ...
    inflation_ratio = args.inflation
    
    planner = create_planner_fn(width, height, resolution, inflation_ratio, goal, args.publish)

    if args.save:
        save_action(task, args.mapname, goal, planner)
